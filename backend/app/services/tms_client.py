...
class TMSClient:
    ...
    def fetch_recent_orders(self, since_date: Optional[str] = None) -> list[ODataOrderPayload]:
        """
        ...
        try:
            ...
            logger.warning(f"1C TMS Sync failed, generating mocked pipeline payload instead: {e}")
    ...
    def update_order_status(self, order_id_1c: str, new_status: str, execution_details: str):
        """
        ...
        payload = {
            "Status": new_status,
            "ExecutionComments": execution_details
        }
        
        try:
            # Note: 1C OData uses PATCH for updates
            headers = {"Content-Type": "application/json"}
            response = self.session.patch(endpoint, json=payload, headers=headers, timeout=5)
            response.raise_for_status()
            logger.info(f"Successfully pushed status '{new_status}' to 1C Order {order_id_1c}")
        except requests.exceptions.RequestException as e:
            logger.warning(f"Could not push to 1C, assuming mocked environment. Error: {e}")
