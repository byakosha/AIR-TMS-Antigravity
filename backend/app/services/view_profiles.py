...
def create_view_profile(db: Session, payload: UserViewProfileCreate) -> UserViewProfile:
    ...
    if profile.is_default:
        db.query(UserViewProfile).filter(UserViewProfile.user_id == profile.user_id).update({"is_default": False})
    db.add(profile)
    db.commit()
    db.refresh(profile)
    return profile
...
def update_view_profile(db: Session, profile_id: int, payload: UserViewProfileUpdate) -> UserViewProfile:
