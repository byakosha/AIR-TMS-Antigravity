...
from app.schemas.planning_rules import (
    AirlineDetailsCreate,
...
def create_airline(
    *,
    ...
    """Create new airline."""
    existing = db.query(AirlineDetails).filter(AirlineDetails.carrier_code == airline_in.carrier_code).first()
    if existing:
        raise HTTPException(status_code=400, detail="Airline with this carrier code already exists.")
...
def create_supply_chain_rule(
    *,
    ...
    existing = db.query(SupplyChainRule).filter(SupplyChainRule.airport_code == rule_in.airport_code).first()
    if existing:
